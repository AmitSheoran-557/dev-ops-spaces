...
const Hero: React.FC<HeroProps> = ({ data }) => {
    ...
    return (
        <div className="flex items-center justify-center 2xl:pt-[139px] xl:pt-24 lg:pt-20 md:pt-14 pt-10 2xl:pb-[62px] xl:pb-12 lg:pb-11 pb-10 bg-fade-white">
            <div className="min-[1600px]:max-w-[1272px] xl:max-w-[1240px] lg:max-w-[960px] md:max-w-[720px] max-w-[540px] px-4 mx-auto w-full">
                <div className="flex flex-wrap items-center justify-between 2xl:pb-11 pb-10">
                    ...
                    <button className="xl:py-[14px] lg:py-3 py-2 xl:px-4 lg:px-3 px-2 text-white font-medium lg:text-base text-sm flex justify-center items-center bg-linear-button rounded-xs">
                ...
                <div className="flex flex-wrap w-full items-start justify-between">
                    ...
                    <div className="lg:w-9/12 w-full flex flex-col">
                                    ...
                                    <span>
                                        <div className="relative w-12 cursor-pointer">
                                            <div className="flex items-center bg-[#A40A86] cursor-pointer min-w-[59px] justify-center gap-1 rounded-md">
                                                <select value={itemsPerPage} onChange={handlePerPageChange} className="bg-[#A40A86] py-1 px-2 relative z-10 outline-none text-white rounded-sm cursor-pointer appearance-none">
                                ...
                                <thead className="bg-[#4F02FE] text-white">
                                    <tr>
                                        {["Code", "Name", "Domains", "Country", "Page"].map((title, index) => (
                                            <th key={index} className="lg:px-[15px] px-3 lg:py-3 py-2 text-left text-xs font-medium uppercase tracking-wider whitespace-nowrap">
                                ...
                                <tbody className="bg-white">
                                    {paginatedData.map((row, index) => (
                                        <tr key={index} className="hover:bg-[#CD0CA714] transition-all ease-linear duration-300">
                                            <td className="lg:px-[15px] px-3 lg:py-3 py-2 text-xs font-medium text-black whitespace-nowrap">{row.alpha_two_code}</td>
                                            <td className="lg:px-[15px] px-3 lg:py-3 py-2 text-xs font-medium text-black whitespace-nowrap">{row.name}</td>
                                            <td className="lg:px-[15px] px-3 lg:py-3 py-2 text-xs font-medium text-black whitespace-nowrap">{row.domains}</td>
                                            <td className="lg:px-[15px] px-3 lg:py-3 py-2 text-xs font-medium text-black whitespace-nowrap">{row.country}</td>
                                            <td className="lg:px-[15px] px-3 lg:py-3 py-2 text-xs font-medium text-black whitespace-nowrap flex justify-between items-center w-full">
                                                <Link href={`${row.web_pages}`} target="_blank" className="text-black/70 hover:text-black transition-all ease-linear duration-300 cursor-pointer flex gap-1">
                                                    {row.web_pages}
                        ...
                        <div className="flex items-center justify-end space-x-2 lg:mt-[33px] mt-7">
                            <button onClick={() => setCurrentPage((prev) => Math.max(prev - 1, 1))} disabled={currentPage === 1} className="bg-white text-light-gray cursor-pointer rounded-lg px-1 py-2 font-semibold lg:text-[13px] text-xs flex items-center justify-center hover:!text-black transition-all ease-linear duration-300">
                                Prev
                            </button>

                            {Array.from({ length: totalPages }, (_, i) => (
                                <button key={i} onClick={() => setCurrentPage(i + 1)} className={`lg:px-3 px-2.5 lg:py-2 py-1.5 rounded-lg !leading-[100%] xl:min-w-8 xl:max-w-8 max-h-8 justify-center flex items-center font-semibold lg:text-[13px] text-xs  ${currentPage === i + 1 ? "bg-gradient-blue !text-white" : "bg-white text-fade-black text-gray-500 hover:bg-gray-200"}`}> {i + 1}
                                </button>
                            ))}

                            <button onClick={() => setCurrentPage((prev) => Math.min(prev + 1, totalPages))} disabled={currentPage === totalPages} className="bg-white text-light-gray cursor-pointer rounded-lg px-1 py-2 font-semibold lg:text-[13px] text-xs flex items-center justify-center hover:!text-black transition-all ease-linear duration-300">
                                Next
                            </button>
                        </div>
                    </div>
